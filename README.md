...
## Roadmap
* During the next days the files `git.lua`, `update.lua` and `mason.lua` from the `base/utils` directory are gonna be removed from NormalNvim, and isolated into the plugin [distroupdate.nvim](https://github.com/Zeioth/distroupdate.nvim). The reason is: It's extremely unlikely a user decide to modify this functionality (which will be still possible, and easier, by forking distroupdate.nvim). This is not a breaking change, it's just a minor cleanup, so if you are about to close the distro, go for it without fear.
