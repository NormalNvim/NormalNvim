...
* ~~curate icon system~~
* bash: we must ship shellcheck as system dependency because that's the only way bashls knows how to use it correctly. (if we install shellcheck from mason, lints will appear duplicated). This is likely to be a regression in bashls, but well, let's fix it anyway. → Note for myself: Don't forget to remove the mason dependency if you change it to system dependency. → Another option is to tell none-ls to ignore shellcheck, so bashls is the responsible of calling it, but this solution has proven to be unreliable as none-ls some times fail to disable shellcheck.
* Prepare promo.
