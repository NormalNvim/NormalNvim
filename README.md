...
| **:NvimUpdateConfig** | Pulls the latest changes from the current git repository of the distro. Useful to keep your config updated when you have your distro installed in more than one machine. If the updates channel is `stable`, this command will pull from the main branch of your github repository. If the channel is `nightly` it will pull from the nightly branch. Note that uncommited local changes in your config will be lost after an update, so it's important you commit before updating your distro config.|
